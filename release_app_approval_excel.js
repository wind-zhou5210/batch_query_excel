...
// // 创建日志写入流
const logStream = fs.createWriteStream('approval_execution_log.txt', { flags: 'a' }); // 'a' 表示追加写入
// 查询详情失败的发布单 写入流
const fialAppStream = fs.createWriteStream('approval_fail_release_list.txt', { flags: 'a' }); // 'a' 表示追加写入
// 记录查询的发布单详情为空的数据 写入流
const emptyAppStream = fs.createWriteStream('approval_empty_release_list.txt', { flags: 'a' }); // 'a' 表示追加写入
// ANSI 转义码：黄色
...
async function main() {
    ...
    saveListToFile(releaselist, 'approval_release_list.json');
    //2.  拼接发布详情数据
    const detailsList = await createDetailList(releaselist);
    console.log('详情列表拼接结束=共有', detailsList.length, '条')
    saveListToFile(detailsList, 'approval_release_detail_list.json');
    //3.  拼接应用信息数据
    ...
    saveListToFile(iterationList, 'approval_release_iteration_list.json');
    //5. 拼接审批id 信息
    const approvalList = await createApprovalList(iterationList);
    console.log('======================审批列表拼接结束=共有', approvalList.length, '条', '======================');
    saveListToFile(approvalList, 'approval_release_iteration_withapproval_list.json');
    ...
    // 10. 对失败的数据重新进行请求
    const result_list = await retryForFailedData();
    const result = [
        ...reviewDetailList,
        ...result_list
    ]
